/* Reliable India WebGIS cell renderer.
 * Logical hierarchy: 1 km² -> 0.01 km² -> 0.0001 km² -> 1 m² -> 0.1 m².
 * The viewport is rendered continuously on a canvas. Millions of logical cells
 * are never created as DOM elements, so the map remains responsive.
 * The authoritative India polygon from PostGIS is the paint clip mask.
 */
(() => {
'use strict';
...
const LEVELS=[
 {name:'1km2',label:'1 km²',side:1000,depth:0,zoom:5},
 {name:'0.01km2',label:'0.01 km²',side:100,depth:1,zoom:9},
 {name:'0.0001km2',label:'0.0001 km²',side:10,depth:2,zoom:13},
 {name:'1m2',label:'1 m²',side:1,depth:3,zoom:17},
 {name:'0.1m2',label:'0.1 m²',side:Math.sqrt(.1),depth:4,zoom:21}
];
const TOKENS=[];
...
function inIndia(lat,lon){if(lat<INDIA_BOUNDS[0][0]||lat>INDIA_BOUNDS[1][0]||lon<INDIA_BOUNDS[0][1]||lon>INDIA_BOUNDS[1][1])return false;return indiaGeometry.some(f=>contains(lat,lon,f))}
...
function cellAt(lat,lon){const level=activeLevel(),[mx,my]=merc(lat,lon),ix=Math.floor((mx-ROOT_X)/1000),iy=Math.floor((my-ROOT_Y)/1000);if(ix<0||iy<0||ix>=ROOT_COLS||!inIndia(lat,lon))return null;let lx=mx-(ROOT_X+ix*1000),ly=my-(ROOT_Y+iy*1000),side=1000,path=[];for(let d=0;d<level.depth;d++){if(d<3){const s=side/10,dx=Math.max(0,Math.min(9,Math.floor(lx/s))),dy=Math.max(0,Math.min(9,Math.floor(ly/s)));path.push(dy*10+dx);lx-=dx*s;ly-=dy*s;side=s}else{const t=Math.sqrt(.1);let hit=-1;for(let k=0;k<10;k++){const p=TERMINAL[k];if(lx>=p[0]&&lx<=p[0]+t&&ly>=p[1]&&ly<=p[1]+t){hit=k;break}}if(hit<0)return null;path.push(hit)}}return{ix,iy,path,code:[rootCode(ix,iy),...path.map(i=>TOKENS[i])].join('-'),level}}
function cellMercator(ix,iy,path){let x=ROOT_X+ix*1000,y=ROOT_Y+iy*1000,side=1000;for(let d=0;d<path.length;d++){const k=path[d];if(d<3){const s=side/10;x+=(k%10)*s;y+=Math.floor(k/10)*s;side=s}else{const p=TERMINAL[k];x+=p[0];y+=p[1];side=Math.sqrt(.1)}}return{x1:x,y1:y,x2:x+side,y2:y+side,side,area:side*side}}
const map=L.map('map',{minZoom:5,maxZoom:25,zoomControl:true,maxBounds:INDIA_BOUNDS,maxBoundsViscosity:.95,zoomAnimation:false,fadeAnimation:false}).fitBounds(INDIA_BOUNDS,{animate:false,padding:[8,8]});
...
const boundaryLayer=L.geoJSON(null,{style:{weight:2,color:'#111827',fill:false,opacity:.9},interactive:false}).addTo(map);
...
function paint(){if(!indiaGeometry.length)return;const size=map.getSize(),d=resizeCanvas(),ctx=canvas.getContext('2d');ctx.setTransform(d,0,0,d,0,0);ctx.clearRect(0,0,size.x,size.y);const z=map.getZoom(),o=map.getPixelOrigin(),v=visibleMerc(),level=activeLevel(),side=level.depth<4?level.side:1,pad=2;let minX=Math.floor((v.minX-side*pad)/side),maxX=Math.ceil((v.maxX+side*pad)/side),minY=Math.floor((v.minY-side*pad)/side),maxY=Math.ceil((v.maxY+side*pad)/side);ctx.save();clipIndia(ctx,z,o);ctx.strokeStyle='rgba(31,78,121,.48)';ctx.lineWidth=1;ctx.beginPath();
 if(level.depth<4){const px=level.side*worldPx(z)/WORLD;const step=px<0.55?Math.max(1,Math.ceil(.55/Math.max(px,.04))):1;for(let gx=minX;gx<=maxX;gx+=step){const x=gx*side,a=screen(x,v.minY-side,z,o),b=screen(x,v.maxY+side,z,o);ctx.moveTo(Math.round(a[0])+.5,0);ctx.lineTo(Math.round(b[0])+.5,size.y)}for(let gy=minY;gy<=maxY;gy+=step){const y=gy*side,a=screen(v.minX-side,y,z,o),b=screen(v.maxX+side,y,z,o);ctx.moveTo(0,Math.round(a[1])+.5);ctx.lineTo(size.x,Math.round(b[1])+.5)}}
 else {for(let gy=minY;gy<=maxY;gy++)for(let gx=minX;gx<=maxX;gx++)for(let k=0;k<10;k++){const p=TERMINAL[k],s=Math.sqrt(.1),a=screen(gx+p[0],gy+p[1],z,o),b=screen(gx+p[0]+s,gy+p[1]+s,z,o),x=Math.min(a[0],b[0]),y=Math.min(a[1],b[1]),w=Math.abs(b[0]-a[0]),h=Math.abs(b[1]-a[1]);if(w>=.5&&x+w>=0&&y+h>=0&&x<=size.x&&y<=size.y)ctx.rect(Math.round(x)+.5,Math.round(y)+.5,Math.max(1,w),Math.max(1,h))}}
...
function parseALU(v){const p=v.trim().toUpperCase().split('-').filter(Boolean);if(p.length<1||p.length>5)throw Error('Enter a 6-character root plus up to four hierarchy segments');const r=rootFromCode(p[0]),path=p.slice(1).map((t,i)=>{if(!TOKEN_TO_INDEX.has(t)||!/^[A-Z0-9]{2}$/.test(t)||!/[A-Z]/.test(t)||!/[0-9]/.test(t))throw Error('Invalid 2-character mixed base36 hierarchy segment');const n=TOKEN_TO_INDEX.get(t);if(i===3&&n>=10)throw Error('Final 0.1 m² refinement has ten terminal children');return n});return{ix:r.ix,iy:r.iy,path,code:[p[0],...p.slice(1)].join('-')}}
document.getElementById('aluSearchForm')?.addEventListener('submit',e=>{e.preventDefault();try{const t=parseALU(searchInput.value),c=cellMercator(t.ix,t.iy,t.path),a=inv(c.x1,c.y1),b=inv(c.x2,c.y2),ll=[[a[0],a[1]],[b[0],b[1]]];highlightLayer.clearLayers();const r=L.rectangle(ll,{weight:4,color:'#ef4444',fill:false,dashArray:'8 6',interactive:false}).addTo(highlightLayer);searchResult.classList.remove('hidden');searchResult.innerHTML='<b>'+t.code+'</b><br>Located ALU cell';map.flyToBounds(r.getBounds().pad(.55),{duration:.9,maxZoom:25})}catch(err){searchHint.textContent=err.message||'Invalid ALU code'}});
async function loadBoundaries(){try{const r=await fetch(API_BASE+'/api/v1/spatial/india-boundaries',{cache:'force-cache'});if(!r.ok)throw Error('boundary '+r.status);const geo=await r.json();indiaGeometry=geo.features||[];boundaryLayer.addData(geo);schedule()}catch(err){console.error(err);searchHint.textContent='Start the FastAPI/PostGIS backend to load the India boundary and cells.'}}
