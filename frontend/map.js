/* India WebGIS — always-visible responsive independent spatial-cell renderer. */
(() => {
'use strict';
...
function inIndia(lat,lon){if(!indiaGeometry.length)return lat>=INDIA_BOUNDS[0][0]&&lat<=INDIA_BOUNDS[1][0]&&lon>=INDIA_BOUNDS[0][1]&&lon<=INDIA_BOUNDS[1][1];return indiaGeometry.some(f=>contains(lat,lon,f))}
...
function paint(){const size=map.getSize(),d=resizeCanvas(),ctx=canvas.getContext('2d');ctx.setTransform(d,0,0,d,0,0);ctx.clearRect(0,0,size.x,size.y);const z=map.getZoom(),o=map.getPixelOrigin(),v=visibleMerc(),level=activeLevel(),unit=level.depth<4?level.side:1,pad=2;let minX=Math.floor((v.minX-unit*pad)/unit),maxX=Math.ceil((v.maxX+unit*pad)/unit),minY=Math.floor((v.minY-unit*pad)/unit),maxY=Math.ceil((v.maxY+unit*pad)/unit);
  ctx.save();
  const clipped=clipIndia(ctx,z,o);
  if(!clipped){
    ctx.beginPath();ctx.rect(0,0,size.x,size.y);
  }
  ctx.strokeStyle='rgba(31,78,121,.50)';ctx.lineWidth=1;ctx.beginPath();
  if(level.depth<4){
    /* Always preserve the grid visually. When a logical cell is sub-pixel,
       multiple logical boundaries are aggregated to the nearest screen pixel;
       as soon as cells become resolvable, every boundary is drawn individually. */
    const px=level.side*worldPx(z)/WORLD;
    const stride=px>=1?1:Math.max(1,Math.ceil(1/Math.max(px,.0001)));
    ...
    for(let gy=minY;gy<=maxY;gy++)for(let gx=minX;gx<=maxX;gx++)for(let k=0;k<10;k++){
      const p=TERMINAL[k],a=screen(gx+p[0],gy+p[1],z,o),b=screen(gx+p[0]+terminal,gy+p[1]+terminal,z,o),x=Math.min(a[0],b[0]),y=Math.min(a[1],b[1]),w=Math.abs(b[0]-a[0]),h=Math.abs(b[1]-a[1]);
      if(w>=.45&&x+w>=0&&y+h>=0&&x<=size.x&&y<=size.y)ctx.rect(Math.round(x)+.5,Math.round(y)+.5,Math.max(1,w),Math.max(1,h))
    }
  }
...
async function loadBoundaries(){try{const r=await fetch(API_BASE+'/api/v1/spatial/india-boundaries',{cache:'force-cache'});if(!r.ok)throw Error('boundary '+r.status);const geo=await r.json();indiaGeometry=geo.features||[];boundaryLayer.addData(geo);schedule()}catch(err){console.error(err);searchHint.textContent='Cells are running in India extent mode. Start FastAPI/PostGIS for exact boundary clipping.';schedule()}}
