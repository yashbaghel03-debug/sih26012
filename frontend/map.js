...
function inIndia(lat,lon){if(lat<INDIA_BOUNDS[0][0]||lat>INDIA_BOUNDS[1][0]||lon<INDIA_BOUNDS[0][1]||lon>INDIA_BOUNDS[1][1])return false;if(!indiaGeometry.length)return true;return indiaGeometry.some(f=>contains(lat,lon,f))}
...
async function loadBoundaries(){try{const r=await fetch(API_BASE+'/api/v1/spatial/india-boundaries',{cache:'force-cache'});if(!r.ok)throw Error('boundary '+r.status);const geo=await r.json();indiaGeometry=geo.features||[];boundaryLayer.addData(geo);schedule()}catch(err){console.error(err);searchHint.textContent='Start FastAPI/PostGIS for exact India boundary clipping; cells still render in India extent mode.';schedule()}}
