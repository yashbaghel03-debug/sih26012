...
function worldPoint(lat,lon,z){const[mx,my]=merc(lat,lon),w=worldPx(z);return[(mx/WORLD+.5)*w,(.5-my/WORLD)*w]}
function buildBoundaryPath(z){const key=Math.round(z*2)/2;if(boundaryPathCache.has(key))return boundaryPathCache.get(key);const path=new Path2D();for(const f of indiaGeometry){const g=f.geometry;if(g?.type==='Polygon'){for(const ring of g.coordinates){if(!ring.length)continue;let p=worldPoint(ring[0][1],ring[0][0],key);path.moveTo(p[0],p[1]);for(let i=1;i<ring.length;i++){p=worldPoint(ring[i][1],ring[i][0],key);path.lineTo(p[0],p[1])}path.closePath()}}else if(g?.type==='MultiPolygon'){for(const poly of g.coordinates)for(const ring of poly){if(!ring.length)continue;let p=worldPoint(ring[0][1],ring[0][0],key);path.moveTo(p[0],p[1]);for(let i=1;i<ring.length;i++){p=worldPoint(ring[i][1],ring[i][0],key);path.lineTo(p[0],p[1])}path.closePath()}}}boundaryPathCache.set(key,path);return path}
...
const street=L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Street_Map/MapServer/tile/{z}/{y}/{x}',{maxZoom:29,maxNativeZoom:19,attribution:'Tiles © Esri'});
...
let indiaGeometry=[],metaCache=new Map(),hoverToken=0,frame=0,lastCanvasW=0,lastCanvasH=0,lastDpr=0,lastHoverCode='',hoverTimer=0,hoverController=null;
function useBoundary(geo){if(!geo?.features?.length)return false;indiaGeometry=geo.features;boundaryPathCache.clear();boundaryLayer.clearLayers();boundaryLayer.addData(geo);return true}
...
function renderTooltip(h,e){tooltip.classList.remove('hidden');tooltip.style.left=Math.min(innerWidth-300,Math.max(10,e.originalEvent.clientX+14))+'px';tooltip.style.top=Math.min(innerHeight-105,Math.max(10,e.originalEvent.clientY+14))+'px';tooltip.innerHTML='<b>'+h.code+'</b><br>Area: '+(h.level.side*h.level.side)+' m²<br>ULPIN: '+(metaCache.get(h.code)?.ulpin||'checking…')}
function hover(e){const h=cellAt(e.latlng.lat,e.latlng.lng);if(!h){lastHoverCode='';tooltip.classList.add('hidden');if(hoverTimer){clearTimeout(hoverTimer);hoverTimer=0}if(hoverController){hoverController.abort();hoverController=null}return}renderTooltip(h,e);if(h.code===lastHoverCode)return;lastHoverCode=h.code;if(hoverTimer)clearTimeout(hoverTimer);if(hoverController)hoverController.abort();hoverTimer=setTimeout(async()=>{hoverTimer=0;if(metaCache.has(h.code)){renderTooltip(h,e);return}hoverController=new AbortController();const t=++hoverToken;try{const r=await fetch(API_BASE+'/api/v1/spatial/cell-info/'+encodeURIComponent(h.code),{cache:'force-cache',signal:hoverController.signal});const d=r.ok?await r.json():{};metaCache.set(h.code,d);if(t===hoverToken)renderTooltip(h,e)}catch(_){if(t===hoverToken){metaCache.set(h.code,{area_m2:h.level.side*h.level.side,ulpin:'Not linked'});renderTooltip(h,e)}}finally{hoverController=null}},80)}
...
map.on('mouseout',()=>{hoverToken++;lastHoverCode='';tooltip.classList.add('hidden');if(hoverTimer){clearTimeout(hoverTimer);hoverTimer=0}if(hoverController){hoverController.abort();hoverController=null}});
...
async function loadBoundaries(){if(loadCachedBoundary()){searchHint.textContent='Indian political boundary: Survey of India / Government of India source.';schedule()}const url=API_BASE+'/api/v1/spatial/india-boundaries';try{const geo=await fetchBoundary(url);if(useBoundary(geo)){try{localStorage.setItem(INDIA_CACHE,JSON.stringify(geo))}catch(_){ }searchHint.textContent='Indian political boundary: Survey of India / Government of India source.';schedule();return}}catch(err){console.warn('Official India boundary source failed',url,err)}searchHint.textContent=indiaGeometry.length?'Using cached official India boundary.':'Official India boundary could not be loaded; grid is withheld to avoid displaying it outside India.';schedule()}
